...
# Extract the first 4 characters of the source file name
source_file_prefix = sys.argv[1][:4]

# Define output file names with prefixes
output_file = f'{source_file_prefix}_01_RG.txt'
output_file2 = f'{source_file_prefix}_02_Norm_Kratky.txt'
...
#####################################
#
# Creation of a .txt file for the VC 
# 
#####################################
...
with open(output_file3, 'w') as file:
        ...
        for i in range(len(q_filtered)):
            file.write(f"{q_filtered[i]}\t{yvc[i]}\n")
...
plt.errorbar(q_range**2, np.log(I_range), yerr=E_range, fmt='o', markersize=4, label='Points expérimentaux')
plt.plot(q_range**2, ln_intensity_theoretical, label='Courbe théorique')
plt.xlabel('q**2')
plt.ylabel('ln(I(q))')
plt.legend()
plt.title('Approximation de Guinier')
plt.grid(True)
plt.show()
